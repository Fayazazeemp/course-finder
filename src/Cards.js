...
function Cards({cn,cs,date,sp,subjectData}) {
    ...
    useEffect(()=>{
      if(subjectData.subjects) {
        setSubjects(subjectData.subjects.slice(0,500));}
...
<div className="count" style={{marginTop:'40px'}}>
...
{
        dataSearch ? <Pagination className="pagination" count={Math.round(dataSearch.length/6)} page={currentPage} onChange={handleChange} color='primary' style={{margin:'auto', marginTop:'40px'}} />:null
    }
